...
class RGBXTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Only Sigmoid Gating modules are now initialized here.
        # self.concat_fuse, self.extra_fuse, self.gates are removed.
        self.thermal_gates = nn.ModuleList()
        self.uv_gates = nn.ModuleList()

        if self.fusion_combination == "sigmoid_gating": # This will be the only path taken
            initial_uv_gate_bias_value = 0.0
            logger.info("Initializing Individual Sigmoid Gating modules for T(idx 1) and UV(idx 2).")
            if not self.use_intensity_enhancement:
                 logger.warning("Sigmoid gating assumes use_intensity_enhancement=True to handle Modality 0 (DIM).")
            if self.n_modal < 2: # DIM, T
                 logger.warning(f"Sigmoid gating expects at least n_modal=2 (DIM, T), but found n_modal={self.n_modal}.")

            for k_idx in range(len(self.embed_dims)):
                channels = self.embed_dims[k_idx]
                if self.n_modal >= 2: # Thermal gate (for modality index 1)
                    self.thermal_gates.append(SigmoidGateModule(base_channels=channels, mod_channels=channels, output_channels=channels))
                else:
                    self.thermal_gates.append(nn.Identity())
                
                if self.n_modal >= 3: # UV gate (for modality index 2)
                    uv_gate_module = SigmoidGateModule(base_channels=channels, mod_channels=channels, output_channels=channels)
                    # Initialize UV Gate Bias
                    gate_mlp_block = uv_gate_module.gate_mlp
                    ...
                    if final_conv_for_uv_bias is not None:
                        nn.init.constant_(final_conv_for_uv_bias.bias, initial_uv_gate_bias_value)
                    else:
                        logger.warning(f"Could not find final Conv2d with bias in UV gate_mlp for stage {k_idx} to initialize bias.")
                    self.uv_gates.append(uv_gate_module)
                else:
                    self.uv_gates.append(nn.Identity())
        else:
            # This case should not be hit if fusion_combination is forced/defaulted to sigmoid_gating
            logger.warning(f"Unexpected fusion_combination '{self.fusion_combination}'. No fusion combination modules initialized.")
            # Ensure lists exist if this path is somehow taken
            if not isinstance(self.thermal_gates, nn.ModuleList): self.thermal_gates = nn.ModuleList([nn.Identity() for _ in embed_dims])
            if not isinstance(self.uv_gates, nn.ModuleList): self.uv_gates = nn.ModuleList([nn.Identity() for _ in embed_dims])
    ...
    def forward_features(self, x_rgb: torch.Tensor, xs: List[torch.Tensor]):
        ...
        # ... (Initial logging and setup as before) ...
        B = x_rgb.size(0)
        outs = [] 
        ...
        for k in range(len(self.depths)): # Loop through stages 0, 1, 2, 3
            ...
            if not modalities_to_fuse:
                ...
                # --- SIGMOID GATING FUSION LOGIC (Now the only main fusion path) ---
                logger.debug(f"      Applying sigmoid_gating fusion...")
                fused = base_feat_for_fusion 
                thermal_contrib = None
                uv_contrib = None

                for i, mod_idx in enumerate(modalities_indices): # modalities_indices are original (0=DIM, 1=T, 2=UV)
                    mod_feat = modalities_to_fuse[i] # Corresponds to xs_img_aligned[mod_idx] effectively
                    if mod_idx == 1: # Thermal Modality (T24 - assumed to be modality at index 1 if n_modal >= 2)
                        if k < len(self.thermal_gates) and self.thermal_gates[k] is not None and not isinstance(self.thermal_gates[k], nn.Identity):
                            thermal_contrib = self.thermal_gates[k](base_feat_for_fusion, mod_feat)
                        else: logger.debug(f"Thermal gate not applied for stage {k} or modality index {mod_idx}")
                    elif mod_idx == 2: # UV Modality (U8 - assumed to be modality at index 2 if n_modal >= 3)
                        if k < len(self.uv_gates) and self.uv_gates[k] is not None and not isinstance(self.uv_gates[k], nn.Identity):
                            uv_contrib = self.uv_gates[k](base_feat_for_fusion, mod_feat)
                        else: logger.debug(f"UV gate not applied for stage {k} or modality index {mod_idx}")
                    # If mod_idx == 0 (DIM), it's not explicitly gated here by thermal/uv_gates.
                    # It's assumed to be handled by use_intensity_enhancement or passed if not consumed.

                # Combine contributions based on sgate_fusion_mode ("add" or "avg")
                if self.sgate_fusion_mode == "add":
                    if thermal_contrib is not None: fused = fused + thermal_contrib
                    if uv_contrib is not None: fused = fused + uv_contrib
                elif self.sgate_fusion_mode == "avg":
                    gated_contributions_list = []
                    if thermal_contrib is not None: gated_contributions_list.append(thermal_contrib)
                    if uv_contrib is not None: gated_contributions_list.append(uv_contrib)
                    if gated_contributions_list:
                        avg_gated_contrib = torch.mean(torch.stack(gated_contributions_list, dim=0), dim=0) if len(gated_contributions_list) > 1 else gated_contributions_list[0]
                        fused = base_feat_for_fusion + avg_gated_contrib # Add average of GATED contributions to base
                    # else: fused remains base_feat_for_fusion
                else: # Fallback for any other sgate_fusion_mode (or if "add"/"avg" were misspelled)
                    logger.warning(f"Sigmoid gating encountered sgate_fusion_mode '{self.sgate_fusion_mode}'. Defaulting to 'add' behavior.")
                    if thermal_contrib is not None: fused = fused + thermal_contrib
                    if uv_contrib is not None: fused = fused + uv_contrib
                # --- END SIGMOID GATING FUSION LOGIC ---
...
# (Code for these functions/classes omitted for brevity but assume they are present as before)
...
class mit_b1(RGBXTransformer): # Assuming mit_b1 might also need similar adjustment later
    def __init__(
        self,
        img_size: int,
        in_chans: int, # Typically 3 for RGB
