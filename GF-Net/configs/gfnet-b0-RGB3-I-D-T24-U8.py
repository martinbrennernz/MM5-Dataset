...
# Dataset config
"""Dataset Path"""
C.dataset_name = 'MM5-FN500' # stratified train - new; no loss adjust - 88.141
...
X_MODALITY_CONFIG = {
    1: { "path": osp.join(DATA_ROOT, "IAIP"),         "align": True },
    2: { "path": osp.join(DATA_ROOT, "D_FocusN"),     "align": True },
...
C.backbone = 'mit_b0SG' # Backbone architecture
...
C.use_intensity_enhancement = False # Use stage-wise RGB+I fusion (StageWiseRGBIntensityFusion)
...
if getattr(C, 'rare_class_multiply', False):
    # Oversampling: multiply rare files according to your config
    file_count_map = {fname: 1 for fname in train_files}
    for idx, file_id in enumerate(C.rare_files_ids):
        fname = f"{file_id}.png"
        if fname in file_count_map:
            file_count_map[fname] = C.rare_files_multiplication[idx]
    expanded_train_files = []
    for fname, count in file_count_map.items():
        expanded_train_files.extend([fname] * count)
