...
class Engine(object):
    ...
    def load_pretrained_weights(self, model, pretrained_dict):
        new_state_dict = OrderedDict()
    ...
    def restore_checkpoint(self):
        if not self.continue_state_object:
            # Skip restoring from checkpoint if continue_state_object is None or empty
            logger.info("No checkpoint to restore (continue_state_object is None).")
            return
        ...
        if 'model' in tmp:
            # Custom function to load weights
            self.load_pretrained_weights(self.state.model, tmp['model'])
            self.state.optimizer.load_state_dict(tmp['optimizer'])
            self.state.epoch = tmp['epoch'] + 1
            self.state.iteration = tmp['iteration']
        else:
            # Assuming tmp is just the model state dictionary
            # Custom function to load weights
            self.load_pretrained_weights(self.state.model, tmp)
            # Optionally reset optimizer, epoch, and iteration if needed
            # self.state.optimizer = [reset or reinitialize optimizer]
            # self.state.epoch = 1
